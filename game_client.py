...
photo_snake = PhotoImage(file=r"snake.gif")
photo_water = PhotoImage(file = r"water.gif")
photo_gun = PhotoImage(file = r"gun.gif")

btn_snake = tk.Button(button_frame, text="Snake", command=lambda : choice("snake"), state=tk.DISABLED, image=photo_snake)
btn_water = tk.Button(button_frame, text="Water", command=lambda : choice("water"), state=tk.DISABLED, image=photo_water)
btn_gun = tk.Button(button_frame, text="Gun", command=lambda : choice("gun"), state=tk.DISABLED, image=photo_gun)
btn_snake.grid(row=0, column=0)
btn_water.grid(row=0, column=1)
btn_gun.grid(row=0, column=2)
...
def enable_disable_buttons(todo):
    if todo == "disable":
        btn_snake.config(state=tk.DISABLED)
        btn_water.config(state=tk.DISABLED)
        btn_gun.config(state=tk.DISABLED)
    else:
        btn_snake.config(state=tk.NORMAL)
        btn_water.config(state=tk.NORMAL)
        btn_gun.config(state=tk.NORMAL)
